...
(function upsiV83PublicLandingAndLanguagePatch() {
  ...
  function v83TeamSection() {
    return `<section class="v83-public-card v83-team-section"><h2>≡ƒæÑ ${esc(tx('teamTitle'))}</h2><div class="v83-team-grid"><article><span>≡ƒæñ</span><strong>${esc(tx('developer1'))}</strong><p>Muhammad Hakimi Adly<br>bin Hazlee<br><b>D20231106511</b></p></article><article><span>≡ƒæñ</span><strong>${esc(tx('developer2'))}</strong><p>Waldan Aiman bin Nazri<br><b>D20231106455</b></p></article><article><span>≡ƒæñ</span><strong>${esc(tx('developer3'))}</strong><p>Muhammad Adib bin Samsuri<br><b>D20231106481</b></p></article><article class="lecturer"><span>≡ƒºæΓÇì≡ƒÅ½</span><strong>${esc(tx('lecturer'))}</strong><p>Profesor Madya Dr. Aslina<br>binti Saad</p></article></div><p class="v83-course-line">≡ƒôû ${esc(tx('course'))}</p></section>`;
  }

  function v83AboutStrip() {
    return `<section class="v83-public-card v83-about-strip"><div><h2>≡ƒûÑ∩╕Å ${esc(tx('aboutWebsite'))}</h2><p>${esc(tx('aboutBody'))}</p></div><div class="v83-values"><article><b>≡ƒ¢í∩╕Å ${esc(tx('reliable'))}</b><span>${esc(tx('reliableText'))}</span></article><article><b>≡ƒîÉ ${esc(tx('accessible'))}</b><span>${esc(tx('accessibleText'))}</span></article><article><b>≡ƒòÿ ${esc(tx('efficient'))}</b><span>${esc(tx('efficientText'))}</span></article></div></section>`;
  }
  ...
  function v83RenderAbout() {
    return `${v83PublicHeader('about')}${v83Hero({ page: 'about', title: esc(tx('aboutWebsite')), intro: tx('aboutHero') })}<main class="v83-public-main">${v83DeclarationPanel()}<section class="v83-info-grid four"><article><span>≡ƒÄ»</span><h3>${esc(tx('purpose'))}</h3><p>${esc(tx('purposeText'))}</p></article><article><span>≡ƒæÑ</span><h3>${esc(tx('who'))}</h3><p>${esc(tx('whoText'))}</p></article><article><span>Γ£à</span><h3>${esc(tx('eduCompany'))}</h3><p>${esc(tx('eduCompanyText'))}</p></article><article><span>≡ƒôû</span><h3>${esc(tx('academicContext'))}</h3><p>${esc(tx('academicContextText'))}</p></article></section><section class="v83-public-card v83-two-col"><div><h2>ΓÜæ ${esc(tx('objectives'))}</h2><ul><li>${esc(tx('obj1'))}</li><li>${esc(tx('obj2'))}</li><li>${esc(tx('obj3'))}</li><li>${esc(tx('obj4'))}</li></ul></div><div><h2>≡ƒôû ${esc(tx('howUse'))}</h2><div class="v83-steps"><article><b>1</b><strong>${esc(tx('stepLogin'))}</strong><p>${esc(tx('stepLoginText'))}</p></article><article><b>2</b><strong>${esc(tx('chooseBook'))}</strong><p>${esc(tx('chooseBookText'))}</p></article><article><b>3</b><strong>${esc(tx('manageBooking'))}</strong><p>${esc(tx('manageBookingText'))}</p></article><article><b>4</b><strong>${esc(tx('enjoyGame'))}</strong><p>${esc(tx('enjoyGameText'))}</p></article></div></div></section></main>${v83Footer()}`;
  }

  function v83FeatureCards() {
    const cards = [['≡ƒÅî∩╕Å', 'driving', 'drivingText'], ['≡ƒôà', 'tee', 'teeText'], ['≡ƒæ¿ΓÇì≡ƒÅ½', 'trainer', 'trainerText'], ['≡ƒòÿ', 'history', 'historyText'], ['≡ƒæñ', 'profile', 'profileText'], ['≡ƒ¢í∩╕Å', 'adminMgmt', 'adminMgmtText'], ['Γ¡É', 'feedback', 'feedbackText']];
  ...
  function v83RenderProject() {
    ...
    return `${v83PublicHeader('project')}${v83Hero({ page: 'project', title: esc(tx('projectInformation')), subtitle: 'UPSI <span>Golf</span> Booking System', intro: tx('projectIntro') })}<main class="v83-public-main"><section class="v83-project-grid top"><article><span>≡ƒôï</span><h3>${esc(tx('projectTitle'))}</h3><strong>${esc(tx('projectTitleValue'))}</strong><p>${esc(tx('projectTitleText'))}</p></article><article><span>≡ƒûÑ∩╕Å</span><h3>${esc(tx('projectType'))}</h3><strong>Web-Based Learning (WBL)</strong><p>${esc(v83Lang === 'ms' ? 'Projek ini dibangunkan sebagai sebahagian daripada projek akademik Web-Based Learning.' : 'This project is developed as part of a Web-Based Learning academic project.')}</p></article><article><span>≡ƒÅ¢∩╕Å</span><h3>${esc(tx('institution'))}</h3><strong>${esc(tx('institutionValue'))}</strong><p>${esc(tx('institutionText'))}</p></article><article><span>≡ƒÄ»</span><h3>Purpose</h3><p>${esc(tx('projectPurposeText'))}</p></article></section><section class="v83-project-grid bottom"><article><span>≡ƒæÑ</span><h3>${esc(tx('targetUsers'))}</h3><ul>${targets.map((item) => `<li>${esc(item)}</li>`).join('')}</ul></article><article><span>≡ƒôï</span><h3>${esc(tx('scope'))}</h3><p>${esc(tx('scopeText'))}</p></article><article class="wide"><span>≡ƒº⌐</span><h3>${esc(tx('modules'))}</h3><ul class="v83-two-list">${modules.map((m) => `<li>${esc(m)}</li>`).join('')}</ul></article></section>${v83DeclarationPanel({ compact: true })}</main>${v83Footer()}`;
  }

...
(function upsiV83PublicLandingAndLanguagePatch() {
  ...
  function v83RenderProject() {
    ...
    return `${v83PublicHeader('project')}${v83Hero({ page: 'project', title: esc(tx('projectInformation')), subtitle: 'UPSI <span>Golf</span> Booking System', intro: tx('projectIntro') })}<main class="v83-public-main"><section class="v83-project-grid top"><article><span>📌</span><h3>${esc(tx('projectTitle'))}</h3><strong>${esc(tx('projectTitleValue'))}</strong><p>${esc(tx('projectTitleText'))}</p></article><article><span>📘</span><h3>${esc(tx('projectType'))}</h3><strong>Web-Based Learning (WBL)</strong><p>${esc(v83Lang === 'ms' ? 'Projek ini dibangunkan sebagai sebahagian daripada projek akademik Web-Based Learning.' : 'This project is developed as part of a Web-Based Learning academic project.')}</p></article><article><span>🏛️</span><h3>${esc(tx('institution'))}</h3><strong>${esc(tx('institutionValue'))}</strong><p>${esc(tx('institutionText'))}</p></article><article><span>🎯</span><h3>Purpose</h3><p>${esc(tx('projectPurposeText'))}</p></article></section><section class="v83-project-grid bottom"><article><span>👥</span><h3>${esc(tx('targetUsers'))}</h3><ul>${targets.map((item) => `<li>${esc(item)}</li>`).join('')}</ul></article><article><span>🧭</span><h3>${esc(tx('scope'))}</h3><p>${esc(tx('scopeText'))}</p></article><article class="wide"><span>🗂️</span><h3>${esc(tx('modules'))}</h3><ul class="v83-two-list">${modules.map((m) => `<li>${esc(m)}</li>`).join('')}</ul></article></section>${v83DeclarationPanel({ compact: true })}</main>${v83Footer()}`;
  }
